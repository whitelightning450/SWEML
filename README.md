...
The SWEML pipeline assimilates nearly 700 snow telemetry (SNOTEL) and California Data Exchange Center (CDEC) sites and combines with processed lidar-derived terrain features for the prediction of a 1 km x 1 km SWE inference in critical snowsheds under 20 minutes on a personal computer.
...
## Inference. 
The model inference is performed the same as model spin-up but with the to-date observations loaded in the SWE_Prediction.ipynb script. 
...
See Figure 4 for an example. This script loads the to-date ground features data (when saved in the appropriate date format), processes the data into the model input feature, and makes predictions. 
...
We validate model performance on the remaining 25% split using RMSE and the coefficient of determination (R2). While running the calibration script, once each regional model is trained, each regional model predicts the data not used in training. 
...
Upon calibration completion, the training script produces a grouped parity plot and barplot to investigate predictive performance, see Figures 7 and 8, respectively. The calibration model saves the best model for each region. 
...
The model weights are stored in a .pkl file that contains a dictionary of dictionaries, with the two key structures of the region, and model layer. 
...
The current iteration of the SWEML makes 20,000 1 km x 1km SWE inferences for select locations throughout the Western U.S.
There is a heavy focus on SWE inferences in the Sierra Nevada mountains, Colorado Rockies, and Wind River Range in Wyoming. 
Once the user initiates Model Run up (i.e., SWE_Initial_Conditions_Prediction), the NWM_SWE_Prediction script makes predictions for the remainder of the water year. 
...
The Data_Processing() function creates a model-friendly data frame for each region to drive each regional ML model (i.e., SWE_Predict()).
...
The plot_interactive() function creates an HTML file to support the interactive exploration of SWE across the western U.S. 
...
Figure 9. The SWEML supports an interactive SWE inference interface to explore how SWE changes with location across the western U.S. 
