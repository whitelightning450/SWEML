...
def Model_train(cwd, epochs, batch_size, RegionTrain, RegionTest, RegionObs_Train, RegionObs_Test):
...
#        X_test.drop(columns = colrem, axis =1, inplace = True)
        X_train.drop(columns = colrem) # For LSTM
        X_test.drop(columns = colrem) # For LSTM
...
#        x = layers.Dense(1)(x)
        ...
        ##========================LTSM========================
        # ValueError: Input 0 of layer "lstm" is incompatible with the layer: expected ndim=3, found ndim=2. Full shape received: (None, 105)
        input_1 = layers.Input(shape=(shape[0], shape[1]))
        x = layers.LSTM(LD1, activation="relu", return_sequences=True)(input_1)
        x = layers.LSTM(LD2, activation="relu", return_sequences=True)(x)
        x = layers.LSTM(LD3, activation="relu", return_sequences=True)(x)
        x = layers.LSTM(LD4, activation="relu", return_sequences=True)(x)
        x = layers.LSTM(LD5, activation="relu", return_sequences=True)(x)
        x = layers.LSTM(LD6, activation="relu")(x)
        x = layers.Dense(1)(x)
