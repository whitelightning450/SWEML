...
def DataProcess(test_year,datapath, cwd):
    ...
    for Region in Region_list:
        ...
        # normalize features
        # scaler = MinMaxScaler(feature_range=(0, 1)) #Shahab commented this
        #save scaler data here too
        # scaled = scaler.fit_transform(df) #Shahab commented this
        # dump(scaler, open(f"{cwd}/Model/{Region}/{Region}_scaler.pkl", 'wb')) #Shahab commented this
        # df = pd.DataFrame(scaled, columns = df.columns)  #Shahab commented this

        #Add Viirs colums
        df = pd.concat([df, dfVIIRS], axis = 1) #Shahab changed from df to df_notscaled
